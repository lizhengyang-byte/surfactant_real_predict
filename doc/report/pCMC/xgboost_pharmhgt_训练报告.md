...
- **XGBoost 以 R²=0.8706 排名第四**，在树模型中排名第三，与 CatBoost（0.8733）差距仅 0.0027
- **XGBoost 投入最大的调参力度**（200 trials + Holdout 二次筛选 + Multivariate TPE），测试性能稳定
- **MLP（Optuna 调参版）以 R²=0.9052 强势登顶**，成为首个突破 0.90 的模型，将此前最佳提升了 3 个百分点
- **Top-4 模型（CIF, CatBoost, XGBoost, MLP）R² 差距仅 0.0072**，四种不同类型模型在性能上高度接近
