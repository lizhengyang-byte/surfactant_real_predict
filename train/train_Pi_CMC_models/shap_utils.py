...
提供：
 ...
 - find_latest_run()         从 runs/Pi_CMC/ 自动定位最新权重目录
...
FEATURE_DIR = os.path.join('data', 'features', 'surfpro', 'Pi_CMC')
RUNS_DIR = os.path.join('runs', 'Pi_CMC')
...
def find_latest_run(model_prefix):
    """在 runs/Pi_CMC/ 中查找该模型最新（按时间戳）的运行目录。
    ...
    """
    pattern = re.compile(rf'^(?:Pi_CMC_)?{re.escape(model_prefix)}_(\d{{8}}_\d{{6}})$')
    candidates = []
