...
提供：
 ...
 - find_latest_run()         从 runs/Area_min/ 自动定位最新权重目录
...
FEATURE_DIR = os.path.join('data', 'features', 'surfpro', 'Area_min')
RUNS_DIR = os.path.join('runs', 'Area_min')
...
def find_latest_run(model_prefix):
    """在 runs/Area_min/ 中查找该模型最新（按时间戳）的运行目录。
    ...
    """
    pattern = re.compile(rf'^(?:Area_min_)?{re.escape(model_prefix)}_(\d{{8}}_\d{{6}})$')
    candidates = []
