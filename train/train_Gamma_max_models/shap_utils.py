...
提供：
 ...
 - find_latest_run()         从 runs/Gamma_max/ 自动定位最新权重目录
...
FEATURE_DIR = os.path.join('data', 'features', 'surfpro', 'Gamma_max')
RUNS_DIR = os.path.join('runs', 'Gamma_max')
...
def find_latest_run(model_prefix):
    """在 runs/Gamma_max/ 中查找该模型最新（按时间戳）的运行目录。
    ...
    """
    pattern = re.compile(rf'^(?:Gamma_max_)?{re.escape(model_prefix)}_(\d{{8}}_\d{{6}})$')
    candidates = []
