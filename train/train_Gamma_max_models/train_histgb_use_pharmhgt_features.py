"""
train_histgb_use_pharmhgt_features.py — HistGradientBoosting with PharmHGT-style Featurization
=============================================================================================
...
HistGradientBoosting: scikit-learn's histogram-based gradient boosting
(近似 LightGBM 的直方图算法，但集成在 sklearn 中，接口更简洁).
...
#     'max_leaf_nodes': 33,
#     'min_samples_leaf': 10,
#     'l2_regularization': 2.680253889236842,
#     'max_bins': 244,
# }


# ===========================================================================
# Main — Load Data, Featurize, Train HistGradientBoosting with Optuna
# ===========================================================================
...
def main():
    ...
    print("=" * 60)
    print("HistGradientBoosting + PharmHGT-style Featurization for Gamma_max Prediction")
    print("=" * 60)
    ...
    if PRETUNED_PARAMS is not None:
        ...
        def objective(trial):
            ...
            for fold, (train_idx_cv, val_idx_cv) in enumerate(kf.split(X_full)):
                ...
                model_cv = HistGradientBoostingRegressor(**params)
        ...
        study = optuna.create_study(
            study_name=f'histgb_{FEATURE_NAME}',
            direction='minimize',
            sampler=sampler,
            pruner=pruner,
        )
    ...
    best_params['early_stopping'] = True         # 最终训练开启早停
    best_params['validation_fraction'] = VAL_FRAC  # 内部划出验证集
    ...
    # ---- Feature Importance (permutation-based, since HistGB lacks native importance) ----
    print(f"\n{'='*60}")
    print("Top 20 Feature Importances (permutation-based)")
    print(f"{'='*60}")
    try:
        from sklearn.inspection import permutation_importance
        perm_imp = permutation_importance(
            final_model, X_val, y_val, n_repeats=5,
            random_state=SEED, scoring='neg_root_mean_squared_error'
        )
        top_idx = np.argsort(perm_imp.importances_mean)[::-1][:20]
        names = FEATURE_NAMES
        for rank, idx in enumerate(top_idx):
            print(f"  {rank+1:2d}. {names[idx]:25s}  {perm_imp.importances_mean[idx]:.4f} ± {perm_imp.importances_std[idx]:.4f}")
    ...
    try:
        ...
        fig.suptitle('HistGradientBoosting + PharmHGT Features - Gamma_max Prediction', fontsize=14)

        # Pred vs True
        ax = axes[0]
        ax.scatter(y_test_orig, y_pred, alpha=0.6, edgecolors='k', linewidth=0.5)
        lims = [min(y_test_orig.min(), y_pred.min()) - 0.5, max(y_test_orig.max(), y_pred.max()) + 0.5]
        ax.plot(lims, lims, 'r--', alpha=0.8, linewidth=1)
        ax.set_xlim(lims); ax.set_ylim(lims)
        ax.set_xlabel('True Gamma_max (mol/m²)'); ax.set_ylabel('Predicted Gamma_max (mol/m²)')
        ...
        ax.set_xlabel('Predicted Gamma_max (mol/m²)'); ax.set_ylabel('Residuals')
    ...
    print(f"\n{'='*60}")
    print("SUMMARY — HistGradientBoosting + PharmHGT Features")
    print(f"{'='*60}")
    ...
    if study_best_value is not None:
        ...
        print(f"  Best CV RMSE: N/A (pretuned)")
    print(f"  Test RMSE: {test_rmse:.4f}")
    print(f"  Test MAE:  {test_mae:.4f}")
    print(f"  Test R²:   {test_r2:.4f}")
    ...
    update_index(run_dir, 'histgb', metrics)
